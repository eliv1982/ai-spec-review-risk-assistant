...
- обязательное production-развертывание: после завершения локального MVP оно было
...
и проверено ранее необязательное production-развертывание: приватное приложение работает
на <https://app.spec-review.elivcloud.org>, публичный портфолио-лендинг — на
<https://spec-review.elivcloud.org>. Инфраструктурная Basic Auth в Traefik ограничивает
доступ к приватному приложению целиком, но не добавляет в продукт прикладную
аутентификацию, пользователей, роли или изоляцию арендаторов и поэтому не изменяет
утвержденные границы продукта.

Публичный лендинг — портфолио/деплой-упаковка, изолированная от backend и API; это не
анонимный публичный доступ к приложению и не новая функция продукта, а отдельная
статическая страница вне review-домена MVP.
