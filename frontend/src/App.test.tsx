...
describe("App — навигация", () => {
  ...
  it("ссылки навигации ведут на точные маршруты", () => {
    ...
    expect(screen.getByRole("link", { name: "Новая проверка" })).toHaveAttribute("href", "/");
    expect(screen.getByRole("link", { name: "Проверки" })).toHaveAttribute("href", "/reviews");
  ...
  it("активный маршрут помечен aria-current=page", () => {
    ...
    expect(screen.getByRole("link", { name: "Проверки" })).toHaveAttribute("aria-current", "page");
    expect(screen.getByRole("link", { name: "Новая проверка" })).not.toHaveAttribute("aria-current");
    expect(screen.getByRole("link", { name: "Журнал аудита" })).not.toHaveAttribute("aria-current");
  });

  it("/reviews: отрисован уникальный заголовок витрины, экран создания документа не отрисован, активна ссылка «Проверки»", () => {
    renderAt("/reviews");

    expect(screen.getByRole("heading", { name: "Проверки", level: 1 })).toBeInTheDocument();
    expect(
      screen.queryByRole("heading", { name: /проверка спецификации ии-ассистентом/i }),
  ...
  it("/audit: отрисован уникальный заголовок журнала аудита, витрина проверок не отрисована, активна ссылка «Журнал аудита»", () => {
    ...
    expect(screen.queryByRole("heading", { name: "Проверки", level: 1 })).not.toBeInTheDocument();
  ...
  it("/reviews/:reviewId: отрисован уникальный экран результата (loading), активна ссылка «Проверки»", () => {
  ...
  it("главная страница по-прежнему показывает форму создания документа", () => {
    ...
    expect(
      screen.getByRole("heading", { name: /проверка спецификации ии-ассистентом/i }),
    ).toBeInTheDocument();
